 ...
 */
 ...
 class MuzSnackbarManager {
    static final int MSG_TIMEOUT = 0;

    private static final int SHORT_DURATION_MS = 1500;
    private static final int LONG_DURATION_MS = 2750;

    private static MuzSnackbarManager sSnackbarManager;

     static MuzSnackbarManager getInstance() {
        if (sSnackbarManager == null) {
            sSnackbarManager = new MuzSnackbarManager();
        }
    ...
    public void show(int duration, Callback callback) {
        synchronized (mLock) {
            ...
            if (mCurrentSnackbar != null && cancelSnackbarLocked(mCurrentSnackbar,
                    MuzSnackbar.Callback.DISMISS_EVENT_CONSECUTIVE)) {
    ...
    private void scheduleTimeoutLocked(SnackbarRecord r) {
        if (r.duration == MuzSnackbar.LENGTH_INDEFINITE) {
        ...
        } else if (r.duration == MuzSnackbar.LENGTH_SHORT) {
    ...
    void handleTimeout(SnackbarRecord record) {
        synchronized (mLock) {
            if (mCurrentSnackbar == record || mNextSnackbar == record) {
                cancelSnackbarLocked(record, MuzSnackbar.Callback.DISMISS_EVENT_TIMEOUT);
            }
        }
    }
